...
(function () {
  ...
  Game_Map.prototype.realDistance = function (x1, y1, x2, y2) {
    return (
      Math.abs(this.realDeltaX(x1, x2)) + Math.abs(this.realDeltaY(y1, y2))
    );
  };
  ...
  Game_CharacterBase.prototype.opacity = function () {
    ...
    return phantomOpacity !== 1
      ? Math.max(Math.floor(opacity * phantomOpacity), param.minimumOpacity)
      : opacity;
  };
  ...
  Game_Event.prototype.initialize = function () {
    ...
    this._visibleDistance =
      parseInt(getMetaValues(this.event(), ["可視距離", "VisibleDistance"])) *
        $gameMap.tileWidth() || 0;
    this._triggerDistance =
      parseInt(
        getMetaValues(this.event(), ["トリガー距離", "TriggerDistance"])
      ) * $gameMap.tileWidth() || 0;
    this._opacityReverse = !!getMetaValues(this.event(), [
      "透明度反転",
      "OpacityReverse",
    ]);
  };
  ...
  Game_Event.prototype.isPhantom = function () {
    return (
      ...
      !this.isValidVisibleSelfSwitch() &&
      !this.isValidVisibleSwitch()
    );
  };
  ...
  Game_Event.prototype.getPhantomOpacity = function () {
    ...
    return Math.min(
      1 - d / ((param.invisibleDistance || 1) * $gameMap.tileWidth()),
      1
    );
  };
